 ...
 *
 * ReactWoo Reviews is on UPDATES_FREE_SLUGS; no license JWT is sent for update checks.
 * Pro/Enterprise features remain gated by GRP_License.
 *
...
/**
 * Registers ReactWoo Reviews with the ReactWoo updates API.
 */
class GRP_Updater {
    const CATALOG_SLUG = 'reactwoo-reviews';
    ...
    public static function get_api_base() {
        $base = apply_filters('grp_updates_api_base', 'https://api.reactwoo.com');
    ...
    public static function filter_update_transient($transient) {
        ...
        $basename = defined('GRP_PLUGIN_BASENAME') ? GRP_PLUGIN_BASENAME : plugin_basename(GRP_PLUGIN_FILE);
        if (!isset($transient->checked[$basename])) {
            return $transient;
        }

        $current = defined('GRP_PLUGIN_VERSION') ? (string) GRP_PLUGIN_VERSION : '';
    ...
    public static function filter_plugins_api($result, $action, $args) {
        ...
        $info               = new stdClass();
        $info->name         = __('ReactWoo Reviews', 'google-reviews-plugin');
        ...
        $info->homepage     = 'https://reactwoo.com/reactwoo-reviews';
        $info->sections     = array(
            'description' => __('Display Google Business reviews on your WordPress site with beautiful widgets and shortcodes.', 'google-reviews-plugin'),
